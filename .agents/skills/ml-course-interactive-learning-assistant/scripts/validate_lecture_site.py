...
class _ContractParser(HTMLParser):
    ...
    def _external_resource(tag: str, attrs: dict[str, str | None]) -> str | None:
        ...
        if tag == "script":
            ...
            candidates.extend((attrs.get("src"), attrs.get("srcset")))

        return next(
            (
                candidate
                for candidate in candidates
                if candidate and NETWORK_URL.match(candidate.strip())
            ),
            None,
        )
...
def validate_html(path: Path) -> list[str]:
    ...
    if not parser.has_graph_fallback:
